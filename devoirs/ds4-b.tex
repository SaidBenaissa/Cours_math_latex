...
\begin{exercice}[Résolution graphique d'(in)équations --- 6 points]
  ...
  \begin{enumerate}
    ...
    \item Déterminer les solutions de $f(x)\leq g(x)$.
    \item Tracer la courbe d'une fonction $h$ telle que les solutions de $h(x)>f(x)$ soient $\left[ -4;0 \right[ \cup \left] 3;6 \right[$.
      \item Pourquoi n'existe-t-il pas de fonction $p$ telle $p(x) \geq g(x)$, et $p(x)\leq0$ sur $\left[ -3;-1 \right]$ ?
  \end{enumerate}
